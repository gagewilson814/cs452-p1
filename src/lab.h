...
typedef struct list {
  ...
  int (*compare_to)(const void *,
                    const void *); /* returns 0 if data are the same*/
 ...
 * that list is storing. The caller must pass the list to list_destroy when
 * finished to free any memory that was allocated.
 *
